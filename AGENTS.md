...
an external program — especially AI coding tools like Codex — the editor reflects
...
- Watch the PR's checks (e.g. `gh pr checks <n> --watch`) and read the Codex-review findings
...
- **Bound the loop:** at most ~3 fix iterations. If checks still aren't green, or a check
  stalls for a **non-code reason** (e.g. Codex-review hitting turn limits, or a transient
...
2. **Escalate instead of self-resolving** when a finding involves: a **design/product
   decision**, a change to or conflict with **AGENTS.md or project policy**, a **data-safety
...
- **Recurring, project-wide convention** (should shape *all* future work) → promote it into
  the relevant section of AGENTS.md as a concise rule.
...
(especially Codex, but any tool — vim, VS Code, `sed`, etc.) edits the open file on
...
External tools like Codex do **NOT** emit cursor positions or an operation stream —
...
- **Visual treatment:** changed-line markers in the gutter, a brief highlight on changed
  regions, and a small author icon (e.g. a Codex/robot glyph) next to externally-changed
  lines. Hovering the icon shows a tooltip like *"Changed by Codex · 2s ago"*.
- **Presence indicator:** detect **write bursts** (rapid successive external writes) to
  drive a status like *"Codex is editing…"* vs *"Codex idle"* in the status bar.
