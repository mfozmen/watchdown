...
- 2026-06-26 — Save-clean via `applyExternalChange`: the renderer marks the session clean after save by calling `applyExternalChange(savedContent)` (correct, since a save is a disk write). When the core is next opened, add an explicit `DocumentSession.markSaved()` so intent is clear and save can't be broken by a future change to the external-change path. From PR #12 review.
